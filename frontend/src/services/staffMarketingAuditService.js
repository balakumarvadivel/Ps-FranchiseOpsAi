...
export const auditService = {
  ...
  resolveFinding: (reportId, resolution) => api.put(`/audits/reports/${reportId}/resolve`, resolution ? { resolution } : {}).then((r) => r.data),
  ...
  submitEvidence: (payload) => api.post("/audits/evidence", payload).then((r) => r.data),
  evidenceFor: (auditId) => api.get(`/audits/${auditId}/evidence`).then((r) => r.data),
  verifyEvidence: (evidenceId, payload) => api.put(`/audits/evidence/${evidenceId}/verify`, payload).then((r) => r.data),
  expiringEvidence: (withinDays = 30) => api.get("/audits/evidence/expiring", { params: { within_days: withinDays } }).then((r) => r.data),
  approvalsFor: (auditId) => api.get(`/audits/${auditId}/approvals`).then((r) => r.data),
  decideApproval: (approvalId, payload) => api.put(`/audits/approvals/${approvalId}/decision`, payload).then((r) => r.data),
};
